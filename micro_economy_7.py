#!/usr/bin/python3
__version__ = '0.0.9' # Time-stamp: <2020-02-15T16:40:54Z>
...
#        if curS <= curPrice.wages / 3:
        ...
        r -= ARGS.score_companies_mag * sum(
        ...
        if ARGS.opt_try == "none":
            ...
            for i in range(10):
                x = [rand1to100(), rand1to100(), rand1to100(), rand1to100(),
                     self.ExpectedStandardProfitRate]
            ...
            res1 = scipy.optimize.dual_annealing \
                (lambda l: self._calcNextState(l)['score'],
                 [(1, 100)] * 4 + [(0.0, 1.0)], x0=self._calcNextInitialValue())
        else:
...
#        st = self._calcNextState(res.x)
        ...
        print("Price W:{0}, N:{1}, L:{2}, I:{3}, S:{4}"
    ...
    def initialize_0 (self):
        ...
        numNotWorking = 300
        ## 前期の価格 p と必需品需要を適当に…。
        p = Price([30, 10, 15, 15, self.ExpectedStandardProfitRate])
        self.prevPrice = p
