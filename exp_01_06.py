#!/usr/bin/python3
__version__ = '0.0.6' # Time-stamp: <2019-05-29T06:47:29Z>
...
parser.add_argument("--opt-try", default="prev", dest="opt_try", choices=["prev", "random", "bh", "bh0", "de", "shgo", "da", "da0"])
parser.add_argument("--score-labor-mag", default=0.01, dest="score_labor_mag", type=float)
parser.add_argument("--score-transform", default="hybrid", dest="score_transform", choices=["log", "log1.1", "exp", "hybrid", "hybrid0.2", "hybrid0.1"])
parser.add_argument("--score-mean", default="max", dest="score_mean", choices=["max", "mean", "each"])

args = parser.parse_args()
TRIALS = args.trials
OPT_TRY = args.opt_try
SCORE_LABOR_MAG = args.score_labor_mag
...
def _score_transform_log_1_1(x):
    ...
    c = 1.1
    b = - a * math.log(c)
    return b + a * math.log(x + c)

def _score_transform_exp(x):
    b = 1
    c = -5  # x = -1 のときの y の値。
    a = math.log(1 - (c/b))
    return - b * math.exp(- a * x) + b

def _score_transform_hybrid(x):
    if x >= 0:
        return 0.2 * math.log(x + 0.1) - 0.2 * math.log(0.1)
    else:
        return - math.exp(- 2 * x) + 1

def _score_transform_hybrid_0_2(x):
    if x >= 0:
        return 0.4 * math.log(x + 0.2) - 0.4 * math.log(0.2)
    else:
        return - math.exp(- 2 * x) + 1

def _score_transform_hybrid_0_1(x):
    if x >= 0:
        return 0.1 * math.log(x + 0.1) - 0.1 * math.log(0.1)
    else:
        return - math.exp(- 2 * x) + 1
...
#        if curS <= curPrice.wages / 3:
        ...
        ## スコアの計算
        r = 0
        r += SCORE_LABOR_MAG * (curDemandOfLabors - curSupplyOfLabors) ** 2
        ...
        if SCORE_MEAN == 'max':
            r -= mean(map(lambda l: score_transform(max(l)), r0))
        elif SCORE_MEAN == 'mean':
            r -= mean(map(lambda l: score_transform(mean(l)), r0))
        else:
            r -= mean([score_transform(item)
    ...
    def step (self):
        ...
        if OPT_TRY == "prev":
            ...
            for p in self.priceHistory:
                ...
                if minScore == None or st['score'] < minScore:
                    minScore = st['score']
                    minArg = p
            ...
            minArg = None
            for i in range(10):
                x = [rand1to1000(), rand1to1000(), rand1to1000(), rand1to1000()]
            ...
            res1 = scipy.optimize.dual_annealing \
                (lambda l: self._calcNextState(l)['score'],
                 [(1, 100)] * 4, x0=self.prevPrice.toArray())
        else:
            raise ValueError("Illegal OPT_TRY: " + OPT_TRY)
