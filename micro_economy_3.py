#!/usr/bin/python3
__version__ = '0.0.7' # Time-stamp: <2020-02-12T12:26:30Z>
...
#        r += 2 * ((curDemandOfLabors - curSupplyOfLabors) ** 2) / (curDemandOfLabors + curSupplyOfLabors)
        ...
        r1 = 0
        for i, c in enumerate(self.necessariesCompanies):
            r1 += self._calcProfitOfCompany(c, p, curPrice, curP[i])
        r_N = r1 / len(self.necessariesCompanies)
        ...
        for i, c in enumerate(self.luxuriesCompanies):
            r1 += self._calcProfitOfCompany(c, p, curPrice, curP[i])
        r_L = r1 / len(self.luxuriesCompanies)
        ...
        for i, c in enumerate(self.ingredientsCompanies):
            r1 += self._calcProfitOfCompany(c, p, curPrice, curP[i])
        r_I = r1 / len(self.ingredientsCompanies)
