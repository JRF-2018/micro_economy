#!/usr/bin/python3
__version__ = '0.0.7' # Time-stamp: <2020-02-12T12:27:13Z>
## Language: Japanese/UTF-8

"""
A simple simulation of micro economics.

「スコア」を求めるとき、必需品・贅沢品・原料を作る企業の利益率の各分野
の平均がバランスを取るように考えてみる。

その上で一定以上に経済が大きくなっていれば、それ以上経済が膨らむのを抑
え、贅沢品の原料率を最大化しようとしてみる。
"""
...
TRIALS = 20
UPDATE_WORKERS_LEVEL = False

def parse_args():
    global TRIALS, UPDATE_WORKERS_LEVEL
    try:
        opts, args = getopt.getopt(sys.argv[1:], "", [
                "trials=", "update-workers-level"
                ])
    except getopt.GetoptError as err:
        print(err)
        print("Usage: " + sys.argv[0] + " --trials=[INT]")
        sys.exit(2)
    for o, a in opts:
        if o == "--trials":
            TRIALS = int(a)
        elif o == "--update-workers-level":
            UPDATE_WORKERS_LEVEL = True
#        elif o == "--update-workers-level":
#            UPDATE_WORKERS_LEVEL = float(a)
        else:
...
class Price:
    def __init__ (self, priceArray):
        ...
        self.ingredientsLevelForLuxuries = priceArray[4]
...
class Economy:
    ...
    def _calcProfitOfCompany (self, company, price, curPrice, num, wL, iL):
        c = company
        cost = (c.fixedAssetsPrice / c.fixedAssetsLife) \
            + (curPrice.wages * c.laborsPerProduct * wL
               + curPrice.ingredients * c.ingredientsPerProduct * iL) * num
        return (price * num - cost) / cost

    def _calcProducts (self, curPrice, price, demand, companies, wL, iL):
        curS = [0 for c in companies]
        delta = Economy.wagesElasticPower \
            * (curPrice.wages - self.prevPrice.wages) / self.prevPrice.wages
        for i, c in enumerate(companies):
            if (price - curPrice.wages * c.laborsPerProduct * wL \
                    - curPrice.ingredients * c.ingredientsPerProduct * iL) > 0:
    ...
    def _scoreOfPOI (self, k, curPrice, price, initDemand, companies, wL, iL):
        if k < 0:
#            return float("inf") ## なぜかエラーになる。
            return initDemand ** 2
        u = k + initDemand
        curS = self._calcProducts(curPrice, price, u, companies, wL, iL)
        s = initDemand
        for i, c in enumerate(companies):
            s += curS[i] * c.ingredientsPerProduct * iL
        return (s - u) ** 2
    
    def _calcProductsOfIngredients (self, curPrice, price, initDemand,
                                    companies, wL, iL):
        maxI = max(map(lambda x: x.ingredientsPerProduct * iL, companies))
        if initDemand <= 0:
            print("ERROR: initDemand must be > 0.")
        res = minimize_scalar(
            lambda k: self._scoreOfPOI(k, curPrice, price, 
                                       initDemand, companies, wL, iL), \
    ...
    def _calcNextState (self, priceArray):
        ...
        if any(map(lambda x: x < 1 or x > 1000, priceArray[0:4])):
            return {'score': float('inf')}
        if any(map(lambda x: x <= 0 or x > 1000, priceArray[4:])):
            return {'score': float('inf')}
...
#        if curS <= curPrice.wages / 3:
        ...
        for i, cp in enumerate(sorted(randomList(self.capitalists),
            ...
            newA = companies + [newC]
            curP = self._calcProducts(curPrice, p, u, newA, wL, iL)
            ## その期の利潤率と3期平均利潤率の低い方が市場利子率より
            ## 高いとき投資する。
            pr = self._calcProfitOfCompany(newC, p, curPrice, curP[-1], wL, iL)
            ...
            if  pr >= Economy.standardProfitRate \
                ...
                newC = None
        if newC == None:
            curP = self._calcProducts(curPrice, p, u, companies, wL, iL)
        ...
        wL = self.workersLevel
        iL = curPrice.ingredientsLevelForLuxuries
        for i, cp in enumerate(sorted(randomList(self.capitalists),
                                      key=self._cp_debt_key)):
            ## 投資のためには debt がないのが条件
            if cp.debt >= 0.1 \
        ...
        wL = self.workersLevel
        iL = 1.0
        iLL = curPrice.ingredientsLevelForLuxuries
        for i, c in enumerate(self.necessariesCompanies):
            u += curProductsOfNecessaries[i] * c.ingredientsPerProduct * iL
        for i, c in enumerate(self.luxuriesCompanies):
            u += curProductsOfLuxuries[i] * c.ingredientsPerProduct * iLL
        ...
        for i, cp in enumerate(sorted(randomList(self.capitalists),
            ...
            curP = self._calcProductsOfIngredients(\
                curPrice, p, u + newC.fixedAssetsIngredients, newA, wL, iL)
            ## その期の利潤率と3期平均利潤率の低い方が市場利子率より
            ## 高いとき投資する。
            pr = self._calcProfitOfCompany(newC, p, curPrice, curP[-1], wL, iL)
            pr2 = sum(prHistory[-2:] + [pr]) / 3
        ...
        for i, c in enumerate(self.necessariesCompanies):
            r += curP[i] * c.laborsPerProduct * wL
        ...
        r += 0.01 * (curDemandOfLabors - curSupplyOfLabors) ** 2
#        r += 2 * ((curDemandOfLabors - curSupplyOfLabors) ** 2) / (curDemandOfLabors + curSupplyOfLabors)
        ...
        r1 = []
        for i, c in enumerate(self.necessariesCompanies):
            r1.append(self._calcProfitOfCompany(c, p, curPrice, curP[i], wL, iL))
        r_N = Economy.standardProfitRate \
            if max(r1) > Economy.standardProfitRate \
            and len(self.necessariesCompanies) > 5 \
            and len(self.luxuriesCompanies) > 5 \
            and len(self.ingredientsCompanies) > 5 \
            else mean(r1)
        ...
        for i, c in enumerate(self.ingredientsCompanies):
            r1.append(self._calcProfitOfCompany(c, p, curPrice, curP[i], wL, iL))
        r_I = Economy.standardProfitRate \
            if max(r1) > Economy.standardProfitRate \
            and len(self.necessariesCompanies) > 5 \
            and len(self.luxuriesCompanies) > 5 \
            and len(self.ingredientsCompanies) > 5 \
        ...
        for i, c in enumerate(self.luxuriesCompanies):
            r1.append(self._calcProfitOfCompany(c, p, curPrice, curP[i], wL, iL))
        r_L = Economy.standardProfitRate \
            if max(r1) > Economy.standardProfitRate \
            and len(self.necessariesCompanies) > 5 \
            and len(self.luxuriesCompanies) > 5 \
            and len(self.ingredientsCompanies) > 5 \
...
#        iL = curPrice.ingredientsLevelForLuxuries
        r += - (iL - 1) * 1.0 \
            if iL > 1 and sum(curP) > 0 and max(r1) > 0 \
            and len(self.necessariesCompanies) > 5 \
            and len(self.luxuriesCompanies) > 5 \
            and len(self.ingredientsCompanies) > 5 \
            else ((iL - 1) ** 2) * 1000
...
#        l = 16 / (3 * softPlusE(0.01, mean([r_N, r_L, r_I])))
        l = 100
#        l = 16 / (3 * softPlus(mean([r_N, r_L, r_I])))
#        l = 16 / (3 * mean([r_N, r_L, r_I]))
        r += - mean([r_N, r_L, r_I]) + l * variance([r_N, r_L, r_I])
    ...
    def step (self):
        ...
        maxPr = None
        for i,c in enumerate(self.necessariesCompanies):
            pr = self._calcProfitOfCompany(c, p, curPrice, curP[i], wL, iL)
        ...
        for i, c in enumerate(self.necessariesCompanies):
            ...
            profit[c.ownerId] = curP[i] * (\
                p - curPrice.wages * c.laborsPerProduct * wL \
                    + curPrice.ingredients * c.ingredientsPerProduct * iL)
        ...
        print("Ingredients Level For Luxuries: {0}"
              .format(curPrice.ingredientsLevelForLuxuries))
        ...
        ## workersLevel の更新
        if UPDATE_WORKERS_LEVEL \
                and len(self.necessariesCompanies) > 5 \
                and len(self.luxuriesCompanies) > 5 \
                and len(self.ingredientsCompanies) > 5:
            self.workersLevel = self.workersLevel * 0.95
